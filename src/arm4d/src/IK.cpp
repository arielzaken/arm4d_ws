...
public:
	...
	: Node("IK_node"), stop_(false)
	{
		...
		// Service that receives incoming step requests
		...
		// Client to talk to Gcode sender
...
private:
	...
	void ikLoop(){
		...
		double l0 = 0.0, l1 = 0.0, l2 = 0.0, l3 = 0.0; // current angles
		bool has_last = false;
		...
		while(!stop_){
			...
			if(step_req.type == 0 || step_req.type == 1){ // only check linear moves
				constexpr double BLOCK_PLANE_Z = 60.0; // height of plane above block
				if(step_req.z < BLOCK_PLANE_Z){
					RCLCPP_ERROR(get_logger(), "Step rejected: would go below ground");
					continue;
				}

				constexpr double BLOCK_PLANE_X = 75.0; // x position of plane in front of block
				if(step_req.x < BLOCK_PLANE_X){
					RCLCPP_ERROR(get_logger(), "Step rejected: would collide with X plane");
					continue;
				}

				constexpr double BLOCK_SPHERE_R = 240.0; // radius of sphere around block
				constexpr double BLOCK_SPHERE_X = shift_x;
				constexpr double BLOCK_SPHERE_Y = shift_y;
				constexpr double BLOCK_SPHERE_Z = L0y;
				double dx = step_req.x - BLOCK_SPHERE_X;
				double dy = step_req.y - BLOCK_SPHERE_Y;
				double dz = step_req.z - BLOCK_SPHERE_Z;
				double dist2 = dx*dx + dy*dy + dz*dz;
				if(dist2 < BLOCK_SPHERE_R*BLOCK_SPHERE_R){
					RCLCPP_ERROR(get_logger(), "Step rejected: would collide with SPHERE");
					continue;
				}

				constexpr double BLOCK_CYLINDER_R = 190.0; // radius of cylinder around block
				constexpr double BLOCK_CYLINDER_H = 190.0; // height of cylinder
				constexpr double BLOCK_CYLINDER_X = shift_x; // centered on robot base
				constexpr double BLOCK_CYLINDER_Y = shift_y;
				constexpr double BLOCK_CYLINDER_Z = 0.0; // on ground level
				dx = step_req.x - BLOCK_CYLINDER_X;
				dy = step_req.y - BLOCK_CYLINDER_Y;
				if(step_req.z < BLOCK_CYLINDER_Z + BLOCK_CYLINDER_H){
					dist2 = dx*dx + dy*dy;
					if(dist2 < BLOCK_CYLINDER_R*BLOCK_CYLINDER_R){
						RCLCPP_ERROR(get_logger(), "Step rejected: would collide with CYLINDER");
						continue;
					}
				}

				constexpr double BLOCK_BOX_X1 = -10.0; // size of box around block
				constexpr double BLOCK_BOX_Y1 = 200.0;
				constexpr double BLOCK_BOX_Z1 = 0.0;
				constexpr double BLOCK_BOX_X2 = 240.0;
				constexpr double BLOCK_BOX_Y2 = 670.0;
				constexpr double BLOCK_BOX_Z2 = 190.0;
				if(step_req.x > BLOCK_BOX_X1 && step_req.x < BLOCK_BOX_X2 &&
				   ...
				   step_req.z > BLOCK_BOX_Z1 && step_req.z < BLOCK_BOX_Z2){
					RCLCPP_ERROR(get_logger(), "Step rejected: would collide with BOX");
					continue;
				}

				constexpr double BLOCK_BOX2_X1 = -70.0; // size of box around block
				constexpr double BLOCK_BOX2_Y1 = -70.0;
				constexpr double BLOCK_BOX2_Z1 = -70.0;
				constexpr double BLOCK_BOX2_X2 =  70.0;
				constexpr double BLOCK_BOX2_Y2 =  70.0;
				constexpr double BLOCK_BOX2_Z2 =  70.0;
				if(step_req.x > BLOCK_BOX2_X1 && step_req.x < BLOCK_BOX2_X2 &&
				   ...
				   step_req.z > BLOCK_BOX2_Z1 && step_req.z < BLOCK_BOX2_Z2){
					RCLCPP_ERROR(get_logger(), "Step rejected: would collide with BOX2, are you sure you are in mm?");
					continue;
				}
			}
			...
			case 1: // G1
			{
				double shifted_x = step_req.x - shift_x;
				double shifted_y = step_req.y - shift_y;
				gcode_req.l0 = 90.0 - atan2(shifted_y, shifted_x) * TO_DEG;

				double u = sqrt(shifted_x * shifted_x + shifted_y * shifted_y) - L0x;
				double v = step_req.z - L0y;
				double alpha = atan2(v, u);
				double r = sqrt(u*u + v*v);
				double beta = acos((L1_2+L2_2-r*r)/(2*L1*L2));
				double gamma = acos((r*r+L1_2-L2_2)/(2*L1*r));

				gcode_req.l1 = (alpha + gamma) * TO_DEG;
				gcode_req.l2 = (alpha + gamma + beta - M_PI) * TO_DEG;

				gcode_req.l3 = step_req.a * TO_DEG - gcode_req.l2;
				if(!has_last || step_req.time <= 0.0){
					gcode_req.type = 0; // rapid
				}
				else{
					gcode_req.type = 1; // linear
					double dl0 = gcode_req.l0 - l0;
					double dl1 = gcode_req.l1 - l1;
					double dl2 = gcode_req.l2 - l2;
					double dl3 = gcode_req.l3 - l3;

					gcode_req.feed = (sqrt(dl0*dl0 + dl1*dl1 + dl2*dl2 + dl3*dl3) * 60 * 1000) / step_req.time; // DEG/min and step_req.time is in ms
				}
				break;
			}
			default:
			...
			// Push to out_queue
			{
				...
				out_queue_.push(gcode_req);
			}
			q_cv_out_.notify_one();
		}
	}
	...
	void txLoop(){
		while(!stop_){
			...
			// Retry loop until service accepts
			bool sent = false;
			while(!sent && !stop_){
				...
				if(future.wait_for(2s) == std::future_status::ready){
					...
					if(resp->success){
						RCLCPP_INFO(get_logger(), "Sent G-code OK");
						sent = true;
					}else{
						RCLCPP_WARN(get_logger(), "G-code rejected, retrying...");
					}
				}else{
					RCLCPP_WARN(get_logger(), "Timeout waiting for G-code service, retrying...");
				}
